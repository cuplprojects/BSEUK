...
const Certificate = ({ data, isPreview }) => {
  ...
  const renderTheoryRows = () => {
    return data.marks
      ...
      .map((mark, index) => (
        <tr key={index}>
          <td style={getCellStyle(mark.code)}>
            {String(mark.code).padStart(3, "0")}
          </td>
      ...
      <tr key={index}>
        {index === 0 && (
          <td
            ...
            style={getCellStyle("अभ्यास क्रम")}
          >
            <div style={{verticalAlign:"top"}}><b> अभ्यास <br /> क्रम</b></div>
          </td>
        )}
        <td style={getCellStyle(mark.name)}>{mark.name}</td>
        <td style={getCellStyle(mark.maxMarks)}>{mark.maxMarks}</td>
        ...
        <td style={getCellStyle(mark.isAbsent ? "A" : mark.practical)}>
          {mark.isAbsent ? "A" : mark.practical}
        </td>
        <td style={getCellStyle(mark.isAbsent ? "A" : mark.total)}>
          {mark.isAbsent ? "A" : mark.total}
        </td>
  ...
  return (
    <div className="p-4 outer-border" style={{ position: "relative" }}>
      ...
      >
        ...
        <div className="text-center mb-3">
          <h5 style={{ fontSize: "1rem", marginBottom: "0.3rem" }}>
            अंक विवरणिका :{" "}
            <span id="sem-hindi">
            ...
            <thead>
              <tr>
                ...
                <th
                  rowSpan="4"
                  className="text-center"
                  style={{ border: "1px solid black", padding: "4px", width: "8%" }}
                >
                  योग
                </th>
              </tr>
